...
### 📌 2. 윌콕슨 부호순위검정 (Wilcoxon Signed-Rank Test)
#### ✅ 개념
- **같은 대상에서 측정한 전후 데이터(쌍을 이룬 데이터)** 비교하는 비모수 검정.
- 대응표본 t-검정(Paired t-test)의 비모수 대안.
- 차이값(전후 데이터의 차이)을 **순위화**하여 분석.

#### ✅ 사용 조건
- **대응표본(쌍을 이룬 데이터, paired samples)**이어야 함.
- 정규성을 따르지 않아도 됨.

#### ✅ 예시
- 같은 환자의 **약물 복용 전후 혈압 비교**.
- 학생들의 **시험 전후 성적 비교**.
...
#### ✅ 개념
- **두 개의 독립적인 그룹** 간 중앙값 차이를 비교하는 비모수 검정.
- 독립표본 t-검정(Independent t-test)의 비모수 대안.
- 두 그룹의 데이터가 같은 분포를 따르는지 검정.

#### ✅ 사용 조건
- **서로 독립적인 두 개의 그룹**이 있어야 함.
- 정규성을 따르지 않아도 됨.

#### ✅ 예시
- 남성과 여성의 **평균 키 비교**.
- A/B 테스트에서 **두 가지 광고 유형의 클릭률 비교**.

#### ✅ R 코드 예시
```r
wilcox.test(group1, group2, paired=FALSE)
```
> `paired=FALSE` 옵션을 사용하여 독립된 두 그룹을 비교.

---

### 📊 차이점 요약

| 검정 방법 | 대응되는 모수 검정 | 데이터 유형 | 비교 그룹 수 | 예시 |
|-----------|-----------------|------------|-------------|------|
| **크루스칼-왈리스 검정** | ANOVA | 독립표본 | **3개 이상** | 약 A, B, C 치료 효과 비교 |
| **윌콕슨 부호순위검정** | 대응표본 t-검정 | 대응표본 | **2개 (전후 비교)** | 같은 환자의 약물 복용 전후 혈압 비교 |
| **만-휘트니 U검정** | 독립표본 t-검정 | 독립표본 | **2개** | 남성과 여성의 키 비교 |

---

### 🎯 결론
- **크루스칼-왈리스 검정**은 **세 개 이상의 독립된 그룹을 비교**하는 경우에 사용됩니다.
- **만-휘트니 U검정**은 **서로 독립적인 두 그룹을 비교**하는 경우에 사용됩니다.
- **윌콕슨 부호순위검정**은 **같은 개체의 전후 비교(쌍을 이룬 데이터)**에 사용됩니다.

👉 즉, **비교할 그룹 수와 데이터 유형(독립/대응)에 따라 적절한 비모수 검정을 선택**해야 합니다!
