...
satisfaction <- data.frame(
  Customer = 1:10,  # 동일한 10명의 고객이 평가
  Brand_A = c(8, 7, 9, 6, 7, 8, 10, 5, 9, 50),  
  Brand_B = c(6, 5, 7, 4, 6, 5, 7, 3, 6, 2),  
  Brand_C = c(7, 8, 6, 5, 6, 9, 8, 6, 5, 40)   
)
...
# 3️⃣ 프리드만 검정 수행
friedman_result <- friedman.test(as.matrix(satisfaction[,2:4]))
...
# p-value < 0.05이면 귀무가설 기각 (브랜드 간 고객 만족도가 차이가 있다고 판단)

# 4️⃣ 시각화 (이상치 포함 박스플롯)
boxplot(satisfaction[,2:4],
        main="패스트푸드 브랜드별 고객 만족도 비교 (프리드만 검정, 이상치 포함)",
        col=c("lightblue", "lightcoral", "lightgreen"),
        ylab="만족도 점수",
...
```
복용 전: 5.2, 4.8, 1.5, 4.5, 4.9, 6.5, 4.7, 4.6
복용 후: 6.1, 5.9, 8.5, 5.5, 6.0, 3.0, 5.6, 5.8
```
</details>
...
# 귀무가설 (H₀): "수면제 복용 전후의 수면 시간 차이는 없다." (수면제 효과가 없다.)
# 대립가설 (H₁): "수면제 복용 전후의 수면 시간 차이가 있다." (수면제 효과가 있다.)

# 2. 데이터 불러오기 

before <- c(5.2, 4.8, 1.5, 4.5, 4.9, 6.5, 4.7, 4.6)
after <- c(6.1, 5.9, 8.5, 5.5, 6.0, 3.0, 5.6, 5.8)
...
# p-value 값이 0.05보다 작으면 귀무가설을 기각하고, 정규성을 따르지 않는다고 판단.
# 정규성을 따르지 않는다면, 대응표본 t-검정 대신 비모수 검정인 윌콕슨 부호순위 검정을 수행해야 함.

# 4. 윌콕슨 부호순위 검정 (비모수 검정)
wilcox.test(before, after, paired = TRUE, alternative = "two.sided")

# 귀무가설 (H₀): "수면제 복용 전후의 수면 시간 차이는 없다." (수면제 효과가 없다.)
# 대립가설 (H₁): "수면제 복용 전후의 수면 시간 차이가 있다." (수면제 효과가 있다.)

# p-value가 0.05보다 작으면 귀무가설을 기각하고, 수면제 효과가 있다고 판단.

# 5. 시각화 하기 

boxplot( list(Before=before, After=after),
         main="수면제 복용 전후 수면 시간 비교 (정규성을 따르지 않는 데이터)",
         col=c("lightblue","lightcoral"),
         ylab="수면 시간 (시간)")
...
```
약국 A: 5200, 15000, 5500, 4900, 5100, 800, 4700, 5000, 5400, 4900
약국 B: 4800, 4500, 12000, 4600, 4700, 4800, 4400, 500, 4900, 4600
```
</details>
...
# 귀무가설 (H₀): "두 약국의 약품 가격 차이는 없다." (약국 간 가격 차이가 없다.)
# 대립가설 (H₁): "두 약국의 약품 가격 차이가 있다." (약국 간 가격에 차이가 있다.)

# 2. 데이터 불러오기 

pharmacy_A <- c(5200, 15000, 5500, 4900, 5100, 800, 4700, 5000, 5400, 4900)
pharmacy_B <- c(4800, 4500, 12000, 4600, 4700, 4800, 4400, 500, 4900, 4600)

# 3. 정규성 검정 하기 

shapiro.test(pharmacy_A)
# 귀무가설: 정규성을 따른다.
# 대립가설: 정규성을 따르지 않는다.

shapiro.test(pharmacy_B)
# 귀무가설: 정규성을 따른다.
# 대립가설: 정규성을 따르지 않는다.

# p-value 값이 0.05보다 작으면 귀무가설을 기각하고, 정규성을 따르지 않는다고 판단.
# 정규성을 따르지 않는다면, 독립표본 t-검정 대신 비모수 검정인 만-휘트니 U 검정을 수행해야 함.

# 4. 만-휘트니 U 검정 (비모수 검정)
wilcox.test(pharmacy_A, pharmacy_B, alternative = "two.sided")

# 귀무가설 (H₀): "두 약국의 약품 가격 차이는 없다." (약국 간 가격 차이가 없다.)
# 대립가설 (H₁): "두 약국의 약품 가격 차이가 있다." (약국 간 가격에 차이가 있다.)

# p-value가 0.05보다 작으면 귀무가설을 기각하고, 두 약국의 약품 가격에 차이가 있다고 판단.

# 5. 시각화 하기 

boxplot( list(A=pharmacy_A, B=pharmacy_B),
         main="약국별 약품 가격 비교 (정규성을 따르지 않는 데이터)",
         col=c("lightblue","lightcoral"),
         ylab="가격")
