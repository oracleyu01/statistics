...
### ✅ 정규성 검정 해석
1. **귀무가설 (H₀)**: 데이터(`before - after`)는 정규성을 따른다.
2. **대립가설 (H₁)**: 데이터(`before - after`)는 정규성을 따르지 않는다.
3. **p-value = 0.6307**은 일반적인 유의수준(0.05)보다 크므로, 귀무가설을 기각할 수 없음.
4. 따라서, `before - after`는 정규성을 만족한다고 볼 수 있음.

---

## 2. 그런데 왜 비모수 검정을 사용할까?
정규성을 만족함에도 **비모수 검정(예: 윌콕슨 부호순위 검정)을 고려하는 이유**는 다음과 같다.
