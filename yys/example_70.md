...
### **📅 2023년 데이터 준분석가 기출문제 1**  
#### **Q1. 온라인 쇼핑몰에서 A 상품과 B 상품의 구매 데이터를 분석한 결과, \( P(A∩B) = 0.15 \), \( P(A) = 0.3 \), \( P(B) = 0.5 \) 일 때, A→B의 신뢰도는 얼마인가?**  
...
**정답: 3) 0.5**  
**해설:**  
신뢰도(Confidence)는 \( P(B|A) \)로 계산됩니다.  

\[
\text{신뢰도} = P(B|A) = \frac{P(A∩B)}{P(A)} = \frac{0.15}{0.3} = 0.5
\]
...
### **📅 2024년 데이터 준분석가 기출문제 2**  
#### **Q2. 마트 거래 데이터에서 우유(A)와 빵(B)의 연관성을 분석했을 때, 지지도가 0.2, \( P(A) = 0.4 \), \( P(B) = 0.5 \)일 때, 향상도는 얼마인가?**  
...
**정답: 2) 1.0**  
**해설:**  
향상도(Lift)는 다음과 같이 계산됩니다.  

\[
\text{향상도} = \frac{P(A∩B)}{P(A) \times P(B)}
\]

\[
\text{향상도} = \frac{0.2}{0.4 \times 0.5} = \frac{0.2}{0.2} = 1.0
\]

향상도가 1.0이면 두 품목은 독립적인 관계를 가짐을 의미합니다.

</details>

---

### **📅 2022년 데이터 준분석가 기출문제 3**  
#### **Q3. 다음 중 연관규칙에서 향상도(Lift)가 0.7일 때의 해석으로 올바른 것은?**  

1. 두 품목은 양의 상관관계가 있다  
2. 두 품목은 독립적인 관계이다  
3. 두 품목은 음의 상관관계가 있다  
4. 두 품목의 관계를 판단할 수 없다  

<details>
<summary><b>정답 보기</b></summary>

**정답: 3) 두 품목은 음의 상관관계가 있다**  
**해설:**  
향상도(Lift)는 다음과 같이 해석됩니다.  

- **향상도 > 1** → 두 품목은 **양의 상관관계** (즉, 함께 구매될 가능성이 높음)  
- **향상도 = 1** → 두 품목은 **독립적인 관계**  
- **향상도 < 1** → 두 품목은 **음의 상관관계** (즉, 함께 구매될 가능성이 낮음)  

따라서 향상도가 **0.7**이면 두 품목은 **음의 상관관계**를 가집니다.
