...
##### **2️⃣ 하단 그래프: 실제 데이터 (평균 μ = 150)**
이제 **Z-값을 실제 데이터(X) 값으로 변환**해야 합니다.
...
##### **📌 표준오차(표본 표준편차) 계산**
\[
SE = \frac{\sigma}{\sqrt{n}} = \frac{12}{\sqrt{100}} = \frac{12}{10} = 1.2
\]
...
✔️ **즉, 151.974 이상이면 귀무가설을 기각할 수 있다!**  

---

## **📌 결론**
- **상단 그래프**에서 **Z = 1.645**가 오른쪽 기각역(α=0.05)의 기준임을 보여줍니다.
- **하단 그래프**에서 이 Z-값을 실제 데이터로 변환하면 **임계값(Critical Value) = 151.974**이 됩니다.
- 따라서 **샘플 평균이 151.974 이상이면 유의수준 5%에서 "평균이 150보다 크다"는 주장을 채택할 수 있습니다.**

---

## **📌 결론**
이 계산 과정은 **단측검정(right-tailed test)**을 가정하고,  
유의수준 5%에서의 **Z-값(1.645)**을 기준으로 **표본평균의 임계값**을 구한 것입니다.
